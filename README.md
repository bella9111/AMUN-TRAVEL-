# AMUN-TRAVEL-
<div align="center">

<img src="Amun travel-20260120T220958Z-1-001/Amun travel/light.png" width="50%" />
<img src="Amun travel-20260120T220958Z-1-001/Amun travel/dark.png" width="50%" />

<br>
# 🌍 AMUN TRAVEL
**Tourism & Hotel Booking Website**

</div>

---

## 🧾 Project Overview
AMUN TRAVEL is a **responsive and interactive tourism & hotel booking frontend**.  
It’s designed to provide users with **smooth booking experience, dynamic search, and intelligent AI suggestions**.

**Tech Stack:**  
HTML | CSS | JavaScript | Ocean AI integration

---

## 🚀 Features

- ✅ **Booking System** – Select tours, hotels, and dates  
- ✅ **Dark & Light Mode** – User-friendly UI themes  
- ✅ **User Profile** – Save bookings and preferences  
- ✅ **Search & Filters** – Quickly find destinations or hotels  
- ✅ **Ocean AI Integration (Coming Soon)** – Smart booking suggestions  
- ✅ **Responsive Design** – Works on mobile, tablet, and desktop  

---

## 📱 Demo
Add screenshots or GIFs of your website here:

<img src="Amun travel-20260120T220958Z-1-001/Amun travel/profile.png" width="45%" />  
<img src="Amun travel-20260120T220958Z-1-001/Amun travel/search.png" width="45%" />
<img src="Amun travel-20260120T220958Z-1-001/Amun travel/contact.png" width="45%" />
---

## 🛠️ Technologies Used

![HTML](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white)
![CSS](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-323330?style=flat&logo=javascript&logoColor=F7DF1E)
![Ocean AI](https://img.shields.io/badge/Ocean_AI-1CA9C9?style=flat&logoColor=white)

---

## 🌟 Key Highlights

- Modern, clean, and responsive **UI/UX**  
- Fully functional **frontend features** ready for backend integration  
- Can be extended with **AI-driven booking suggestions**  
- Ideal as a **portfolio project** to showcase frontend skills  

---

## 📫 Contact / More Info
- GitHub: **[your-github-username]**  
- Portfolio: *(optional)*  

---

<div align="center">

**Code Cycle**  

<img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Smilies/Face%20with%20Spiral%20Eyes.png" width="8%" />
&nbsp;&nbsp;
<img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Smilies/Relieved%20Face.png" width="8%" />
&nbsp;&nbsp;
<img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Smilies/Astonished%20Face.png" width="8%" />

</div>
